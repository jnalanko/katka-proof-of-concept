...
void traverse_subtree(const string& tree_encoding, int64_t left, int64_t right){

    cout << "Subtree " << tree_encoding.substr(left, right-left+1) << endl;
    ...
    if(tree_encoding[left] == '('){ // Internal node
        ...
        cout << "Internal node name: " << name << endl;
        ...
        while(true){
            ...
            if(tree_encoding[branch_start] == '('){
                ...
                traverse_subtree(tree_encoding, subtree_start, subtree_end);
            } else{
                // Subtree is a leaf
                ...
                if(colon_pos != -1){
                    ...
                    cout << "Length: " << tree_encoding.substr(colon_pos + 1, branch_end - (colon_pos+1) + 1) << endl;
                }
                traverse_subtree(tree_encoding, subtree_start, subtree_end);
            }
        ...
        // Leaf → Name
        cout << "Leaf name: " << tree_encoding.substr(left, right-left+1) << endl;
    }
}
...
int main(int argc, char** argv){
    ...
    traverse_subtree(tree_encoding, 0, tree_encoding.size()-1-1); // Discard the ';' in the end.
}